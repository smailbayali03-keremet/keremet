      ...
      <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(135deg, #FAFAF8 0%, #FFF8ED 100%)' }}>
        <div className="elite-card p-10 max-w-md w-full text-center">
          ...
          <p className="text-[#6B6B6B] mb-6 leading-relaxed">
            Дизайнер сіздің өтінішіңізді алды. Жақын арада орындалады.
          </p>
          <div className="p-4 rounded-xl bg-[#FEF9EC] border border-[#F0D98A]">
            <p className="text-sm text-[#B7821A] font-medium">Өтініш нөміріңізді сақтаңыз</p>
            <p className="text-xs text-[#6B6B6B] mt-1 font-mono">{token?.slice(0, 8)}...</p>
          </div>
        </div>
      </div>
    )
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-12">
        <div className="elite-card p-8 max-w-lg w-full">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              <label className="block text-sm font-semibold text-[#1A1A1A] mb-2">
                Кабинет нөмірі <span className="text-[#C9A84C]">*</span>
              </label>
              <input
                type="text"
                value={cabinet}
            ...
            <div>
              <label className="block text-sm font-semibold text-[#1A1A1A] mb-2">
                Не керек? <span className="text-[#C9A84C]">*</span>
              </label>
              <div className="grid grid-cols-2 gap-2">
                {REQUEST_TYPES.map(type => (
                  <button
                    ...
                    style={{
                      border: requestType === type ? '2px solid #C9A84C' : '1.5px solid #E8E0D0',
                      background: requestType === type ? '#FEF9EC' : 'white',
                      color: requestType === type ? '#A07830' : '#1A1A1A',
                    }}
                  >
                    {type}
                  </button>
            ...
            <div>
              <label className="block text-sm font-semibold text-[#1A1A1A] mb-2">
                Жеделдік <span className="text-[#C9A84C]">*</span>
              </label>
              <div className="flex gap-3">
                <button
                  ...
                  style={{
                    border: urgency === 'urgent' ? '2px solid #C9A84C' : '1.5px solid #E8E0D0',
                    background: urgency === 'urgent' ? '#FEF9EC' : 'white',
                    color: urgency === 'urgent' ? '#A07830' : '#1A1A1A',
                  }}
                >
                  🔥 Срочно (1-2 сағат)
                </button>
                <button
                  type="button"
                  onClick={() => setUrgency('anytime')}
                  className="flex-1 py-3 px-4 rounded-lg border text-sm font-medium transition-all"
                  style={{
                    border: urgency === 'anytime' ? '2px solid #C9A84C' : '1.5px solid #E8E0D0',
                    background: urgency === 'anytime' ? '#FEF9EC' : 'white',
                    color: urgency === 'anytime' ? '#A07830' : '#1A1A1A',
                  }}
                >
                  🕐 Кез келген уақытта
                </button>
            ...
            <div>
              <label className="block text-sm font-semibold text-[#1A1A1A] mb-2">
                Қосымша ескерту
                <span className="text-[#6B6B6B] font-normal ml-2">(міндетті емес)</span>
              </label>
              <textarea
                value={notes}
